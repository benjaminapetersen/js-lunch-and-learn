$(function() {
  ...
  var makePersonDom = function(item, $tpl, index) {
    ...
    html = html.replace(/\{index\}/g, index || ''); // need to use a regex w /g for multple...
    return html;
  };

  var renderAll = function(list, $tpl, $container) {
    $container.empty();
    var toAppend = '';
    if(list.length) {
      list.forEach(function(item, i) {
        toAppend = toAppend + makePersonDom(item, $tpl, i);
      });
    } else {
  ...
  var makeEditable = function(list, $container) {
    ...
    $('.edit', $container).click(function(e) {
      ...
      renderAll(list, $personTpl, $container);
  ...
  var $showPerson = $('#show-person .target');


  // renderAll once
  // -------------------------------------------------------------- */
  renderAll(people, $personTpl, $peopleContainer);
  ...
  // $('base').attr('href', 'http://localhost:8000/src/');
  // page.base('/src');
  // // now setup routes
  // // btw, routes can have multple callbacks:
  // // page('/foo/bar', load, show, stuff);
  // // $peopleContainer, $form, $showPerson
  // page('/', function() {
  //   console.log('index!', $peopleContainer);
  //   $peopleContainer.show();
  //   $form.show();
  //   $showPerson.hide();
  // });
  // page('/user/:user', function(context) {
  //   // show
  //   console.log('show!', context.params.user);
  //
  // });
  // page('/user/:user/edit', function(context) {
  //   // edit
  //   console.log('edit!', context.params.user);
  // });
  // page('*', function() {
  //   console.log('404!', '');
  // });
